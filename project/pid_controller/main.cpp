...
void path_planner(vector<double>& x_points, vector<double>& y_points,
  ...
  goal =
      behavior_planner.state_transition(ego_state, goal, is_junction, tl_state);
...
int main() {
  ...
  /**
   * TODO (Step 1): create pid (pid_steer) for steer command and initialize
   * values
   **/
  ...
  pid_steer.Init(0.3, 0.001, 1.0, 1.2, -1.2);  // 1.2 and -1.2 are the output limits for the steer command, and are given in the rubrik
  pid_throttle.Init(0.5, 0.0, 0.0, 1.0, -1.0);  // 1.0 and -1.0 are the output limits for the throttle command, and are given in the rubrik
